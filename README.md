...
- Web 控制台：录制时实时显示 RGB、深度伪彩和 IMU 状态，转换时显示阶段进度、
  最新提取帧与局部 PLY；全部使用本机 11920 单口
...
2. 点击“开始录制”，页面会显示实时 RGB、0.3–3 米深度伪彩、采集帧率和传感器
   状态。Azure Kinect 同时显示真实加速度计、陀螺仪、温度和 IMU 采样率；扫描场景后
   点击“结束录制”，页面会等待 MKV/BAG 安全封装。
3. 点击“开始转换”，页面先显示最新提取的对齐 RGB-D 帧，再在局部片段生成后切换为
   可旋转的中间点云，同时展示 `extract → make → register → refine → integrate` 进度。
   程序会执行完整的
   `make → register → refine → integrate` 重建。
4. 完成后可在页面旋转、缩放采样后的点云预览，并下载完整
...
浏览器预览约以 4 FPS 更新，用于取景和质量检查，不会降低录制文件的目标帧率。
Azure Kinect 的 Web 录制使用项目内 K4A SDK 同一设备句柄同时写入 RGB-D capture
和 IMU track，预览不会再次连接或争抢相机。实时深度显示在原始深度坐标系；转换后
的数据集仍按重建要求对齐到彩色图像。
